...
export const useCleanBody = () => {
  ...
  const cleanProspectBody = async (prospect, campaignId) => {
    ...
    if (prospect.clean_body && prospect.clean_body !== "") {
      ...
      if (!prospect.label) {
        try {
          ...
          console.log('Classification obtenue:', classifyResponse);
          ...
          console.log('Classification sauvegardée dans localStorage:', updatedCampaigns);
        } catch (err) {
    ...
    try {
      ...
      campaignsFacade.save(updatedCampaigns);
      console.log('Données sauvegardées dans localStorage:', updatedCampaigns);
